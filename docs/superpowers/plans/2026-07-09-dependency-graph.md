# Dependency Graph Visualization — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Hiển thị task dependency graph (dagre layout + SVG render) trong PlanReview (static/priority mode) và MissionDashboard (live/status mode). Shared component `PlanDependencyGraph.jsx`.

**Architecture:** Install `@dagrejs/dagre`. Tạo `PlanDependencyGraph.jsx` dùng `useMemo` để compute layout, SVG để render. Thêm tab "Graph" vào PlanReview và MissionDashboard.

**Tech Stack:** React 19, `@dagrejs/dagre`, SVG, Tailwind CSS, Lucide icons

## Global Constraints

- Chỉ thêm `@dagrejs/dagre`, không thêm react-flow hoặc d3
- `depends_on` là array of task title strings — resolve title → task.id để build edges
- Isolated tasks (không depends_on, không ai depend vào) vẫn render trong graph
- Node width: 180px fixed
- Không break PlanReview drag-and-drop, MissionDashboard tabs hiện có
- UI text: tiếng Việt
...
- **Modify:** `src/components/mission/PlanReview.jsx` — thêm tab "Graph"
- **Modify:** `src/components/mission/MissionDashboard.jsx` — thêm tab "Graph"

---

### Task 1: Install @dagrejs/dagre + tạo PlanDependencyGraph component

**Files:**
- Create: `src/components/mission/PlanDependencyGraph.jsx`

**Interfaces:**
- Produces: `<PlanDependencyGraph tasks={Task[]} mode={'plan'|'live'} onNodeClick={(task) => void} />`
...
- [ ] **Step 3: Verify renders without crash**

```bash
npm run dev
```

Import component vào một trang test và pass 3-5 tasks với depends_on để kiểm tra graph render.

- [ ] **Step 4: Commit**

```bash
git add src/components/mission/PlanDependencyGraph.jsx package.json package-lock.json
git commit -m "feat: add PlanDependencyGraph component with dagre layout and SVG render"
```

---

### Task 2: Thêm tab "Graph" vào PlanReview

**Files:**
- Modify: `src/components/mission/PlanReview.jsx`

**Interfaces:**
- Consumes: `PlanDependencyGraph` từ Task 1

- [ ] **Step 1: Tìm tab switcher trong PlanReview**

```bash
grep -n "tab\|activeTab\|Visual\|Document\|setActiveTab\|onTabChange" src/components/mission/PlanReview.jsx | head -20
```

Hiểu cách tab state được quản lý (state nội bộ hay prop từ parent).
...
- [ ] **Step 3: Thêm "Graph" vào tab switcher**

Tìm tab button list (ví dụ `['visual', 'document']`), thêm `'graph'`:

```jsx
// Trong tab button array hoặc map:
...
<button
  ...
  className={`px-3 py-1.5 text-xs font-mono rounded transition-colors ${
    activeTab === 'graph'
      ...
      : 'border border-vs-border text-vs-muted hover:text-vs-text'
  }`}
>
  Graph
</button>
...
- [ ] **Step 4: Render graph khi tab active**

Trong conditional render của tabs:
...
{activeTab === 'graph' && (
  <div className="flex-1 overflow-hidden">
    <PlanDependencyGraph
      ...
      mode="plan"
      onNodeClick={(task) => {
        // Scroll to task in list — nếu có ref hoặc setSelectedTask
        // Nếu không có mechanism, no-op là acceptable
      }}
    />
  </div>
)}
...
- Tab "Graph" xuất hiện cạnh Visual/Document
- Click tab Graph → graph hiện
- Nodes màu theo priority
- Edges render đúng hướng
- Drag-and-drop trong tab Visual vẫn hoạt động sau khi switch từ Graph

- [ ] **Step 6: Commit**

```bash
git add src/components/mission/PlanReview.jsx
git commit -m "feat: add Graph tab to PlanReview with static dependency visualization"
```

---

### Task 3: Thêm tab "Graph" vào MissionDashboard
...
- [ ] **Step 1: Tìm tab structure trong MissionDashboard**

```bash
grep -n "activeTab\|tab\|Tasks\|Agents\|Log\|Files\|Messages" src/components/mission/MissionDashboard.jsx | head -25
```

Hiểu tab list và render pattern.
...
- [ ] **Step 3: Thêm 'graph' vào tab list**

Tìm array tabs (ví dụ `['tasks', 'agents', 'log', 'files', 'messages']`), append `'graph'`.

Tìm tab button render, thêm button cho Graph với cùng styling pattern.

- [ ] **Step 4: Render graph tab content**
...
{activeTab === 'graph' && (
  <div className="flex-1 overflow-hidden h-full">
    <PlanDependencyGraph
      ...
      mode="live"
      onNodeClick={(task) => {
        // Switch sang tab tasks và highlight task đó
        setActiveTab('tasks')
        // nếu có setHighlightedTask hay scroll mechanism, dùng ở đây
      }}
    />
  </div>
)}
...
- Tab "Graph" xuất hiện trong MissionDashboard
- Nodes màu theo status (pending/in_progress/completed)
- `in_progress` nodes có pulse animation
- Agent name hiện dưới task title trong node
- Khi task status thay đổi (simulate hoặc run mission) → graph tự update
- Click node → switch sang tab Tasks
...
git commit -m "feat: add Graph tab to MissionDashboard with live dependency visualization"
```