...
**IMPORTANT**: Analyze the list of skills  at `%USERPROFILE%/.claude/skills/*` and intelligently activate the skills that are needed for the task during the process.
...
Example:
```bash
node %USERPROFILE%/.claude/scripts/set-active-plan.cjs ai_docs/feature/GH-88-add-authentication
```
