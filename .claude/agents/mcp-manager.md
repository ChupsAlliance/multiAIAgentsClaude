...
**IMPORTANT**: Analyze skills at `%USERPROFILE%/.claude/skills/*` and activate as needed.
...
[ ! -f .gemini/settings.json ] && mkdir -p .gemini && ln -sf %USERPROFILE%/.claude/.mcp.json .gemini/settings.json
...
```bash
npx tsx %USERPROFILE%/.claude/skills/mcp-management/scripts/cli.ts call-tool <server> <tool> '<json-args>'
```
