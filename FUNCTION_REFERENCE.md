...
### 21. retry_agent(agentName)

**Purpose**: Reset an error'd agent to Idle and re-queue its task for Lead to re-spawn

**Parameters**:
- `agentName` (String): Name of the agent to retry (must match `agent.name` in missionState)
...
```typescript
{ ok: true }                              // Success
{ ok: false, error: "No active mission" } // No mission
{ ok: false, error: "Agent not found" }   // Agent name mismatch
{ ok: false, error: "No retryable task" } // Agent has no assigned task
```

**Behavior**:
1. Finds agent in `missionState.agents` by name
2. Sets `agent.status = 'Idle'`, clears `agent.error`
3. Finds the agent's most recent task with status `'in_progress'` or `'completed'`
4. Sets `task.status = 'pending'`
5. Emits `mission:agent-spawned` (agent object, no `reset` flag) so frontend updates agent card
6. Logs a System entry visible in PlanningStream / ActivityLog
7. Writes a stdin prompt to Lead process asking it to re-spawn the agent

**Frontend side:** `retryAgent(agentName)` in `useMission` calls this command. Shows `toast.info` on success, `toast.error` if `ok === false` or if the invoke throws.

**Example**:
```javascript
const result = await window.__TAURI__.invoke('retry_agent', { agentName: 'backend-api' });
if (!result.ok) {
  console.error(result.error); // "Agent not found" etc.
}
```

**Note**: Only meaningful when the mission is still running (Lead process active). If Lead is already done, the stdin write is a no-op.

---

### 22. answer_question(answers)

**Purpose**: Send user answers back to Lead in Interactive permission mode

**Parameters**:
- `answers` (Object): `{ [questionIndex]: string }` map of question answers

**Return Type**: `void`

**Behavior**: Writes `<<<ANSWER>>> ... <<<END_ANSWER>>>` block to Lead's stdin.

---

### 23. mockup_respond(action, feedback)

**Purpose**: Approve or request revision of a mockup generated by Lead (Deep Plan)

**Parameters**:
- `action` (String): `'approve'` or `'revise'`
- `feedback` (String): Revision notes (only meaningful when `action === 'revise'`)

**Return Type**: `void`

**Behavior**: Writes the response to Lead's stdin and emits `mission:answer-sent`.
...
**Purpose**: Send current plan back to AI for revision, receive updated plan

**Parameters**:
- `agents`: Current agent array
- `tasks`: Current task array

**Returns**: Promise<`{ agents, tasks }`> — revised plan

---

##### `retryAgent(agentName)`

**Purpose**: Reset an error'd agent and notify Lead to re-spawn it

**Parameters**:
- `agentName` (String): The agent's `.name` field

**Returns**: Promise<void> — shows toast on success or failure

**Example**:
```javascript
await retryAgent('backend-api');
// Toast appears: "Đang retry agent 'backend-api'" on success
```
